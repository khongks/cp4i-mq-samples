...
data:
  ${NAME}.mqsc: |
    DEFINE QL(SINKQ)
    DEFINE QL(APPQ) STREAMQ(SINKQ) STRMQOS(BESTEF)
    DEFINE CHANNEL(${QMGR_NAME}CHL) CHLTYPE(SVRCONN) REPLACE TRPTYPE(TCP) SSLCAUTH(OPTIONAL) SSLCIPH('TLS_RSA_WITH_AES_256_CBC_SHA256')
...
if [ $phase == Running ]; then
   echo Queue Manager ${NAME} is ready; 
   exit; 
fi
